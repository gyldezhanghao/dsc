import ajax from "./ajax";
//定义一个全局变量--全局路径
const Base_URL = "/api"

//请求数据
//首页-轮播图
export const getHomeSwipe = () => ajax(Base_URL + "/v1/index/swipe");
//分类-商品分类
export const getCategoryList = () => ajax(Base_URL + "/v1/category");
//分类--商品数据
// export const getCategoryRight = (cid) => ajax(Base_URL + `/v1/category/categorylist/${cid}`);
//登录
export const loginuser = (user_name, pass, captcha) => ajax(Base_URL + "/v1/users/login", { user_name, login_password: pass, captcha }, "post");
//产品列表页面
//api/v1/category/goodslist?cat_id=1106
export const getGoodsList = (params) => ajax(Base_URL + "/v1/category/goodslist", params);
//产品详情页
///api/v1/category/goodsdetail?goods_id=621
export const getGoodsDetail = (params) => ajax(Base_URL + "/v1/category/goodsdetail", params);
//搜索页面
//api/v1/search?keywords=手机&size=5&page=1
export const searchFn = (params) => ajax("http://39.101.210.136:3000/api/v1/search", params);